    ...
    <div style={{ padding: '20px' }} className="w-full">
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr>
            ...
            <th className="border p-2">Month of Sale</th>
        ...
        <tbody>
          {currentProducts.map((product) => (
            <tr key={product.id}>
              ...
              <td className="border p-2">{product.monthOfSale}</td>
              <td className="border p-2">${product.price.toFixed(2)}</td>
              <td className="border p-2">{product.sold ? 'Yes' : 'No'}</td>
      ...
      <div className="flex justify-between items-center mt-4">
        <button
          ...
          className="px-4 py-2 bg-blue-500 text-white rounded-sm hover:bg-blue-700 shadow-md disabled:opacity-50"
        >
          Previous
        </button>
        <span>
